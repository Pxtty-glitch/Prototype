/* ================================================================
   LOCAL-ONLY PROTOTYPE ADAPTER
   No Firebase, backend, API server, or database is required.
   Data is stored in this browser's localStorage.
   ================================================================ */
(function installLocalPrototypeAdapter(){
  const DB_KEY = 'lfd-lms.prototype.db.v1';
  const SESSION_KEY = 'lfd-lms.prototype.session.v1';
  const defaultDb = () => ({
    users: [
      {id:'local-admin', fullName:'Administrator', email:'admin@lfd.local', password:'admin1234', role:'ADMINISTRATOR'},
      {id:'local-staff', fullName:'Staff User', email:'staff@lfd.local', password:'staff1234', role:'STAFF'}
    ],
    clients: [], loans: [], payments: [],
    settings: {companyName:'La Familia DAVID Lending',tin:'',address:'',interestRatePercent:3,penaltyRatePercent:2,gracePeriodDays:3,loanTypes:['Business Loan','Salary Loan','Group Loan'],autoBackup:true,cloudSync:false}
  });
  function read(){try{return JSON.parse(localStorage.getItem(DB_KEY))||defaultDb()}catch(e){return defaultDb()}}
  function write(db){localStorage.setItem(DB_KEY,JSON.stringify(db));return db}
  function uid(prefix){return prefix+'-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,8)}
  let db=read(); if(!db.users?.length) db=write(defaultDb());
  const findId=(path)=>decodeURIComponent(path.split('/').filter(Boolean).pop()||'');
  const firebaseShim={
    auth:{Auth:{Persistence:{LOCAL:'LOCAL'}},EmailAuthProvider:{credential:(email,password)=>({email,password})}},
  };
  let listeners=[];
  function current(){const id=localStorage.getItem(SESSION_KEY); return db.users.find(u=>u.id===id)||null}
  const auth={
    currentUser: current(),
    async signInWithEmailAndPassword(email,password){
      db=read(); const u=db.users.find(x=>x.email.toLowerCase()===String(email).toLowerCase()&&x.password===password);
      if(!u){const e=new Error('Incorrect email or password.');e.code='auth/invalid-credential';throw e}
      localStorage.setItem(SESSION_KEY,u.id); auth.currentUser={uid:u.id,email:u.email,displayName:u.fullName}; listeners.forEach(fn=>fn(auth.currentUser)); return {user:auth.currentUser};
    },
    async signOut(){localStorage.removeItem(SESSION_KEY);auth.currentUser=null;listeners.forEach(fn=>fn(null));},
    async setPersistence(){return true},
    onAuthStateChanged(fn){listeners.push(fn); setTimeout(()=>fn(auth.currentUser),0); return ()=>{listeners=listeners.filter(x=>x!==fn)}}
  };
  window.firebase=firebaseShim; window.firebaseAuthInstance=auth;
  window.api={
    async get(path){db=read();
      if(path==='/clients') return db.clients||[];
      if(path==='/loans') return db.loans||[];
      if(path==='/payments') return db.payments||[];
      if(path==='/settings') return db.settings||defaultDb().settings;
      if(path==='/auth/me'){const u=current(); if(!u) throw new Error('Not authenticated'); return {id:u.id,fullName:u.fullName,email:u.email,role:u.role};}
      if(path==='/users') return (db.users||[]).map(({password,...u})=>u);
      if(path==='/reports/dashboard') return {};
      throw new Error('Local prototype route not implemented: '+path);
    },
    async post(path,payload={}){db=read();
      if(path==='/auth/register'){
        if(db.users.some(u=>u.email.toLowerCase()===String(payload.email).toLowerCase())) throw new Error('An account with this email already exists.');
        const u={id:uid('user'),fullName:payload.fullName||'User',email:payload.email,password:payload.password,role:payload.requestedRole==='STAFF'?'STAFF':'STAFF'}; db.users.push(u); write(db); return {id:u.id,fullName:u.fullName,email:u.email,role:u.role};
      }
      if(path==='/clients'){const c={...payload,id:payload.id||uid('client'),registrationDate:payload.registrationDate||new Date().toISOString().slice(0,10),status:payload.status||'CURRENT'};db.clients.push(c);write(db);return c;}
      if(path==='/loans'){const l={...payload,id:payload.id||uid('loan'),status:payload.status||'CURRENT',principal:Number(payload.principal||0),principalPaid:0,interestPaid:0,totalInterest:Number(payload.totalInterest||0),totalReceivable:Number(payload.totalReceivable||payload.principal||0),outstandingBalance:Number(payload.outstandingBalance||payload.totalReceivable||payload.principal||0)};db.loans.push(l);write(db);return l;}
      if(path==='/payments'){const p={...payload,id:uid('payment'),refId:'R-'+Date.now(),date:payload.date||new Date().toISOString().slice(0,10),voided:false,archived:false};db.payments.push(p);const loan=db.loans.find(l=>String(l.id)===String(payload.loanId));if(loan){const amt=Number(payload.amount||0);loan.principalPaid=Number(loan.principalPaid||0)+amt;loan.outstandingBalance=Math.max(0,Number(loan.outstandingBalance||loan.totalReceivable||loan.principal||0)-amt);if(loan.outstandingBalance<=0.009)loan.status='FULLY_PAID';}write(db);return p;}
      if(/^\/loans\/[^/]+\/approve$/.test(path)){const l=db.loans.find(x=>String(x.id)===findId(path.replace('/approve','')));if(l)l.status='CURRENT';write(db);return l||{};}
      if(/^\/payments\/[^/]+\/void$/.test(path)){const p=db.payments.find(x=>String(x.id)===findId(path.replace('/void','')));if(p)p.voided=true;write(db);return p||{};}
      throw new Error('Local prototype route not implemented: '+path);
    },
    async put(path,payload={}){db=read();
      if(path==='/settings'){db.settings={...db.settings,...payload};write(db);return db.settings;}
      const m=path.match(/^\/(clients|loans)\/([^/]+)$/); if(m){const arr=db[m[1]]||[];const i=arr.findIndex(x=>String(x.id)===m[2]);if(i>=0){arr[i]={...arr[i],...payload};write(db);return arr[i]};throw new Error('Record not found');}
      throw new Error('Local prototype route not implemented: '+path);
    },
    async del(path){db=read();const m=path.match(/^\/clients\/([^/]+)$/);if(m){db.clients=(db.clients||[]).filter(x=>String(x.id)!==m[1]);write(db);return {ok:true}}throw new Error('Local prototype route not implemented: '+path)}
  };
  window.__resetLfdPrototype=()=>{localStorage.removeItem(DB_KEY);localStorage.removeItem(SESSION_KEY);location.reload()};
})();
...
/* ======================================================================
   DATA LAYER — localStorage prototype adapter (no backend)
   ====================================================================== */
...
// Keep local prototype persistence so an authenticated session survives a full
